...
/** Known Garden pest head textures and the name shown by the highlighter. */
public enum PestKind {
	...
	private static final Map<String, PestKind> BY_TEXTURE = buildLookup();

	private final String displayName;
	private final List<String> textures;

	PestKind(String displayName, String... textures) {
	...
	/** Resolves a raw Minecraft player-head texture value without accepting partial matches. */
	public static Optional<PestKind> fromTexture(String texture) {
		if (texture == null || texture.isBlank()) return Optional.empty();
		return Optional.ofNullable(BY_TEXTURE.get(texture));
	}

	/** Number of raw texture variants, including Earthworm and Firefly aliases. */
	public static int textureCount() {
		return BY_TEXTURE.size();
	}

	static Set<String> knownTextures() {
		return BY_TEXTURE.keySet();
	}
	...
	private static Map<String, PestKind> buildLookup() {
		Map<String, PestKind> lookup = new HashMap<>();
		for (PestKind kind : values()) {
			for (String texture : kind.textures) {
				lookup.put(texture, kind);
			}
		}
